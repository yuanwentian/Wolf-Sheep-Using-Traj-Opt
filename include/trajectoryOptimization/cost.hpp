	...
	class GetControlSquareSum {
		...
		public:
			...
			double operator()(const double* trajectoryPointer) const {
				...
				const auto addToDistanceSquareSum = [&] (const unsigned index)
				 {
				 	const auto nowPosition = trajectoryPointer + index * pointDimension;
				 	distanceSquareSum += 1/(std::pow((nowPosition[0]-initialPositionX), 2));
					distanceSquareSum += 1/(std::pow((nowPosition[1]-initialPositionY), 2));
				 };
				 std::for_each(numberOfPointsRange.begin(), numberOfPointsRange.end(), addToDistanceSquareSum);

				//return distanceSquareSum+controlSquareSum;
				return distanceSquareSum + controlSquareSum;
			}  
	};
}//namespace
